 ...
 */
function applyImageCompression(image, encoderType, encoderOptions = {}) {
  return image[encoderType](encoderOptions)
}
...
export async function encodeImageData(bitmap, fullWidth, fullHeight, options) {
  const {encoderType, encoderOptions, targetWidth, targetHeight = null, targetFit = 'inside', preEncodeHook} = options
  ...
  await applyImageCompression(image, encoderType, encoderOptions)
